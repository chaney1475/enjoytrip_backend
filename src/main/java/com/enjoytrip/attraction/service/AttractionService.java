...
public class AttractionService {
    ...
    private final AttractionUserMapper attractionUserMapper;
    ...
    public AttractionDto insertAttraction(AttractionCreateCommand command) {
        ...
        if(!attractionUserMapper.findByAttractionIdAndUserId(attraction.getAttractionId(), command.getUserId()).isEmpty()) {
            throw new AttractionException("USER_ATTRACTION_ALREADY_EXIST", "관심목록에 관광지가 이미 존재", "관심목록에 관광지가 이미 존재합니다.");
        }

        result = attractionUserMapper.createAttractionUser(attraction, command.getUserId());
        if(result == 0) {
    ...
    public List<AttractionDto> getUserAttractions(Long userId) {
        ...
        return attractionUserMapper.findByUserId(userId)
                .stream().map(AttractionDto::from).toList();
    }

    @Transactional
    public void deleteUserAttraction(Long attractionId, Long userId) {

        int result = attractionUserMapper.deleteByAttractionIdAndUserId(attractionId, userId);
    ...
    public List<TopNthAttractionDto> getTopNthAttractions() {
        ...
        return attractionMapper.findTopNth(TOP_N)
